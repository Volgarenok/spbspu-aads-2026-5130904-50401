...
template <class Key, class Value> struct NodeHashTable {
  ...
  NodeHashTable &operator=(NodeHashTable &&other) noexcept {
    if (this != &other) {
  ...
  NodeHashTable(const NodeHashTable &) = default;
  NodeHashTable &operator=(const NodeHashTable &) = default;
};

template <class Key, class Value, class Hash = Sha1Hasher<Key>,
...
public:
  ...
  using reference = typename std::conditional<IsConst, const value_type &,
                                              value_type &>::type;
  using list_iterator =
      typename std::conditional<IsConst, LCIter<NodeHashTable<Key, Value>>,
  ...
  HashTableIterator(const HashTable<Key, Value, Hash, EqualTo> *table,
                    size_t index, list_iterator overflow_it = nullptr) noexcept
