...
BOOST_AUTO_TEST_CASE(test_read_end_of_stream)
{
  ...
  readToken(ss);
  Token t = readToken(ss);

  BOOST_CHECK_EQUAL(t.type, TokenType::End);
}

BOOST_AUTO_TEST_CASE(test_number_overflow)
{
  std::stringstream ss("99999999999999999999");
  BOOST_CHECK_THROW(readToken(ss), std::overflow_error);
}
...
{
  std::stringstream ss("123a @ ?");

  BOOST_CHECK_THROW(readToken(ss), std::runtime_error);
